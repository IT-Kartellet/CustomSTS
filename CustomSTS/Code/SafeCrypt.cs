...
namespace CustomSTS.Code
{
    ...
    public class SafeCrypt
    {
        ...
        public SafeCrypt()
        {
            ...
            byte[] encryptionKey = SoapHexBinary.Parse(ConfigurationManager.AppSettings["EncryptionKey"]).Value;
            byte[] validationKey = SoapHexBinary.Parse(ConfigurationManager.AppSettings["ValidationKey"]).Value;
            initAlgorithms(encryptionKey, validationKey);
        }
