...
namespace LDAPUtils
{
    ...
    {
        public static LdapConnection LdapConnectBind(Uri url)
        {
            ...
            NetworkCredential credential = new NetworkCredential("anonymous", "");
            ...
            catch (LdapException ex)
            {
                ...
                if (ex.ErrorCode == 49)
                {
                    throw new Exception("Anonymous find failed");
                }
                else
                {
                    throw;
        ...
        {
            // Validate that user and password has been set
            if (String.IsNullOrEmpty(user) || String.IsNullOrEmpty(password))
            {
                throw new Exception("Both username and password has to have a value when doing non-anonymous bind");
            }
            ...
            catch (LdapException ex)
            {
                ...
                if (ex.ErrorCode == 49)
                {
                    throw new Exception("Invalid credentials for user " + user);
                }
                else
                {
                    throw;
